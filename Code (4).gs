...
// ── HTTP ROUTER (GET) ─────────────────────────────────────────────
// ALL requests use GET to avoid CORS preflight errors when hosted on
// GitHub Pages / Netlify. Owner panel sends full payload as JSON in
// the ?payload= param. Staff app sends simple ?action=&branchId= params.
function doGet(e) {
  try {
    // ── OWNER PANEL: full JSON payload sent as GET param ──────────
    // Avoids CORS preflight — owner panel encodes entire request body
    // as ?payload=<urlencoded JSON> so browser never sends OPTIONS preflight
    if (e.parameter.payload) {
      var raw = e.parameter.payload;
      // Handle double-encoding that some browsers apply
      try { raw = decodeURIComponent(raw); } catch(ex2) {}
      var d = JSON.parse(raw);
      // Route to the correct function using the action field
      switch (d.action) {
        case "ownerLogin":       return R(ownerLogin(d));
        case "adminLogin":       return R(adminLogin(d));
        case "getBranches":      return R(getBranches());
        ...
        case "masterFix":        return R(masterFix(d));
        case "getLastUpdate":    return R(getLastUpdate(d));
        default: return E("Unknown action: " + d.action);
      }
    }

    // ── STAFF APP: simple ?action=&branchId= GET params ───────────
    var a   = e.parameter.action;
    var bid = e.parameter.branchId;
    switch (a) {
    ...
    return E("doGet error: " + ex.message);
  }
}
